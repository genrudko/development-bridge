...
Before commit, the successor also performs a native-listener preflight: the successor model must call `coordinator_x_mount` for the reserved channel and Browser Host must mechanically query the resulting coordinator MCP App for that exact pending channel with `observer_only=false`. Assistant text and Browser Host's observer-only control binding are never accepted as preflight proof. The private rollover nonce remains entirely outside model-visible prompts and results.

Only after candidate URL/ID, control binding, native mount, and X polling all pass does Bridge atomically commit the new route generation. The post-commit bootstrap is delivered by Bridge's configured direct wake transport under the route-specific mutation lock and resumes from `coordinator_route_context_get`. Automatic Browser Host rollover now requires `coordinator_wake_delivery.enabled` with a configured transport: the commit request checks this prerequisite before changing the active route, and Browser Host aborts back to the source if it is unavailable. The private rollover token stays in server control requests, never the bootstrap prompt. A durable send-attempt marker prevents blind retries after uncertain delivery or process interruption; only a proven `not_submitted` result retries automatically. Uncertain attempts require receipt reconciliation before completion. Failures before commit abort the pending rollover and restore the old physical conversation; the old route remains canonical throughout verification. Manual takeover is rejected while an automatic rollover is pending.
