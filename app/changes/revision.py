...
class ChangeRevisionCalculator:
    ...
    async def calculate(self, repository: Repository) -> str:
        ...
        tracked_drift = await self._runner.run(
            repository, ["diff-files", "--name-only", "-z", "--"]
        )
        untracked = await self._runner.run(
            repository,
        ...
        relevant_paths = {
            path
            for output in (tracked_drift.stdout, untracked.stdout)
        ...
        for relative_path in sorted(relevant_paths):
            ...
            path = relative_path.encode("utf-8")
            ...
            if candidate.is_symlink():
                ...
                digest.update(b"missing" if not candidate.exists() else b"other")
