from __future__ import annotations

from collections.abc import Mapping
from enum import StrEnum
from typing import Any


class ErrorCode(StrEnum):
    INVALID_ARGUMENT = "INVALID_ARGUMENT"
    PROJECT_NOT_FOUND = "PROJECT_NOT_FOUND"
    REPOSITORY_NOT_FOUND = "REPOSITORY_NOT_FOUND"
    REPOSITORY_CONFLICT = "REPOSITORY_CONFLICT"
    REPOSITORY_CLONE_FAILED = "REPOSITORY_CLONE_FAILED"
    MANAGED_REPOSITORY_STATE_CORRUPT = "MANAGED_REPOSITORY_STATE_CORRUPT"
    GITHUB_NOT_CONFIGURED = "GITHUB_NOT_CONFIGURED"
    GITHUB_REPOSITORY_UNAVAILABLE = "GITHUB_REPOSITORY_UNAVAILABLE"
    GITHUB_API_ERROR = "GITHUB_API_ERROR"
    GITHUB_RATE_LIMITED = "GITHUB_RATE_LIMITED"
    GITHUB_CONFLICT = "GITHUB_CONFLICT"
    PERMISSION_DENIED = "PERMISSION_DENIED"
    POLICY_VIOLATION = "POLICY_VIOLATION"
    GIT_REVISION_NOT_FOUND = "GIT_REVISION_NOT_FOUND"
    GIT_COMMAND_FAILED = "GIT_COMMAND_FAILED"
    GIT_INDEX_EMPTY = "GIT_INDEX_EMPTY"
    GIT_OPERATION_IN_PROGRESS = "GIT_OPERATION_IN_PROGRESS"
    GIT_PUSH_PLAN_INVALID = "GIT_PUSH_PLAN_INVALID"
    GIT_PUSH_REJECTED = "GIT_PUSH_REJECTED"
    GIT_BRANCH_EXISTS = "GIT_BRANCH_EXISTS"
    GIT_BRANCH_NOT_FOUND = "GIT_BRANCH_NOT_FOUND"
    GIT_UPSTREAM_NOT_CONFIGURED = "GIT_UPSTREAM_NOT_CONFIGURED"
    GIT_WORKTREE_DIRTY = "GIT_WORKTREE_DIRTY"
    GIT_FAST_FORWARD_REJECTED = "GIT_FAST_FORWARD_REJECTED"
    IDEMPOTENCY_CONFLICT = "IDEMPOTENCY_CONFLICT"
    REVISION_CONFLICT = "REVISION_CONFLICT"
    CHANGE_PRECONDITION_FAILED = "CHANGE_PRECONDITION_FAILED"
    CHANGE_PLAN_INVALID = "CHANGE_PLAN_INVALID"
    CHANGE_APPLY_FAILED = "CHANGE_APPLY_FAILED"
    TASK_NOT_FOUND = "TASK_NOT_FOUND"
    JOB_NOT_FOUND = "JOB_NOT_FOUND"
    JOB_NOT_CANCELLABLE = "JOB_NOT_CANCELLABLE"
    JOB_EXECUTION_NOT_CONFIGURED = "JOB_EXECUTION_NOT_CONFIGURED"
    JOB_BUSY = "JOB_BUSY"
    ARTIFACT_NOT_FOUND = "ARTIFACT_NOT_FOUND"
    ARTIFACT_EXPORT_NOT_CONFIGURED = "ARTIFACT_EXPORT_NOT_CONFIGURED"
    KNOWLEDGE_NOT_CONFIGURED = "KNOWLEDGE_NOT_CONFIGURED"
    KNOWLEDGE_SOURCE_NOT_FOUND = "KNOWLEDGE_SOURCE_NOT_FOUND"
    KNOWLEDGE_MESSAGE_NOT_FOUND = "KNOWLEDGE_MESSAGE_NOT_FOUND"
    KNOWLEDGE_ATTACHMENT_NOT_FOUND = "KNOWLEDGE_ATTACHMENT_NOT_FOUND"
    KNOWLEDGE_ATTACHMENT_CORRUPT = "KNOWLEDGE_ATTACHMENT_CORRUPT"
    KNOWLEDGE_EXPORT_NOT_CONFIGURED = "KNOWLEDGE_EXPORT_NOT_CONFIGURED"
    TELEGRAM_NOT_CONFIGURED = "TELEGRAM_NOT_CONFIGURED"
    TELEGRAM_AUTHORIZATION_REQUIRED = "TELEGRAM_AUTHORIZATION_REQUIRED"
    TELEGRAM_SOURCE_NOT_FOUND = "TELEGRAM_SOURCE_NOT_FOUND"
    TELEGRAM_RATE_LIMITED = "TELEGRAM_RATE_LIMITED"
    TELEGRAM_REQUEST_FAILED = "TELEGRAM_REQUEST_FAILED"
    CHATGPT_SHARE_FETCH_FAILED = "CHATGPT_SHARE_FETCH_FAILED"
    CHATGPT_SHARE_HTTP_ERROR = "CHATGPT_SHARE_HTTP_ERROR"
    CHATGPT_SHARE_TOO_LARGE = "CHATGPT_SHARE_TOO_LARGE"
    CHATGPT_SHARE_UNSUPPORTED_FORMAT = "CHATGPT_SHARE_UNSUPPORTED_FORMAT"
    DESKTOP_NODE_NOT_CONFIGURED = "DESKTOP_NODE_NOT_CONFIGURED"
    DESKTOP_NODE_NOT_FOUND = "DESKTOP_NODE_NOT_FOUND"
    DESKTOP_NODE_OFFLINE = "DESKTOP_NODE_OFFLINE"
    DESKTOP_NODE_BUSY = "DESKTOP_NODE_BUSY"
    DESKTOP_NODE_TIMEOUT = "DESKTOP_NODE_TIMEOUT"
    NO_ACTIVE_DESIGN = "NO_ACTIVE_DESIGN"
    WRONG_DOCUMENT = "WRONG_DOCUMENT"
    REF_STALE = "REF_STALE"
    REF_SPLIT = "REF_SPLIT"
    REF_AMBIGUOUS = "REF_AMBIGUOUS"
    TYPE_MISMATCH = "TYPE_MISMATCH"
    SELECTOR_EMPTY = "SELECTOR_EMPTY"
    SELECTOR_AMBIGUOUS = "SELECTOR_AMBIGUOUS"
    PRECONDITION_FAILED = "PRECONDITION_FAILED"
    UNSUPPORTED_GEOMETRY = "UNSUPPORTED_GEOMETRY"
    CAPABILITY_UNAVAILABLE = "CAPABILITY_UNAVAILABLE"
    CAPABILITY_DEGRADED = "CAPABILITY_DEGRADED"
    VIEW_STALE = "VIEW_STALE"
    FUSION_API_ERROR = "FUSION_API_ERROR"
    VALIDATION_FAILED = "VALIDATION_FAILED"
    TRANSACTION_CONFLICT = "TRANSACTION_CONFLICT"
    CHECKPOINT_DIVERGED = "CHECKPOINT_DIVERGED"
    SAVE_CONFIRMATION_REQUIRED = "SAVE_CONFIRMATION_REQUIRED"
    OPERATION_UNCERTAIN = "OPERATION_UNCERTAIN"
    INTERNAL_ERROR = "INTERNAL_ERROR"


class BridgeError(Exception):
    def __init__(
        self,
        code: ErrorCode,
        message: str,
        *,
        retryable: bool = False,
        details: Mapping[str, Any] | None = None,
    ) -> None:
        super().__init__(message)
        self.code = code
        self.message = message
        self.retryable = retryable
        self.details = dict(details or {})


class ToolNameConflictError(RuntimeError):
    pass
