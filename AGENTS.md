...
- Fusion CAD executors must also read and follow `docs/operations/fusion-cad-executor-guide.md`. Modeling tasks are result-first: complete and verify the requested artifact, treat safety/capability rules as constraints rather than separate work, switch implementation after a failed channel, and report a blocker only after materially applicable permitted paths are exhausted. Public `fusion_*` tools remain preferred when supported; explicitly permitted raw Fusion/Autodesk execution is a bounded fallback, not a reason to stop or ask the owner to run code manually.
...
- **Model-visible physical identity prohibition:** Never ask the owner to paste or copy a physical ChatGPT conversation URL. Never request, echo, paste, or transfer a physical conversation URL, `conversation_id`, `project_id`, MCP/session identity, bind/rollover/control token, nonce, `redirectUrl`/return target, or equivalent physical binding material through model-visible chat, prompts, model messages, or delegated `bridge_call` arguments. Logical `route_id` values and safe diagnostic IDs are allowed.
- Native mobile new-bind is currently limited by the ChatGPT host/WebView because the client does not supply the return target. Existing bound routes may still be controlled from mobile. Use desktop/Web for a new physical binding; do not reintroduce marker/search discovery as a mobile fallback.
